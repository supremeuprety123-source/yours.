...
import { todayISO, formatTime, formatDate } from '../lib/utils';
import Modal from '../components/Modal';
import ConfirmDialog from '../components/ConfirmDialog';
import EmptyState from '../components/EmptyState';
import { Plus, Edit2, Trash2, Play, Pause, Square, Timer, Clock, Flame, TrendingUp, Calendar, Check, ChevronLeft, ChevronRight, Target, Milestone } from 'lucide-react';
...
export default function Planner() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-[#1A1A1A] p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <button onClick={() => { const d = new Date(selectedDate + 'T00:00'); d.setDate(d.getDate() - 1); setSelectedDate(d.toISOString().split('T')[0]); }} className="rounded-lg border border-zinc-700 p-1.5 text-zinc-400 hover:text-zinc-200"><ChevronLeft size={16} /></button>
              ...
              <button onClick={() => { const d = new Date(selectedDate + 'T00:00'); d.setDate(d.getDate() + 1); setSelectedDate(d.toISOString().split('T')[0]); }} className="rounded-lg border border-zinc-700 p-1.5 text-zinc-400 hover:text-zinc-200"><ChevronRight size={16} /></button>
      ...
      <ConfirmDialog open={!!deleteTarget} title="Delete Routine" message={`Delete "${deleteTarget?.title}"? This cannot be undone.`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
    </div>
  );
}
